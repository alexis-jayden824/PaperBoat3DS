...
build_directory=${1:-"$project_root/build/m13-scene-tests"}
host_cc=${HOST_CC:-cc}
...
test_binary="$build_directory/test_world_scene"

if [ ! -f "$zlib_root/inflate.c" ]; then
    echo "M13 world-scene test: fetch pinned upstream sources first" >&2
    exit 1
fi
...
"$host_cc" -std=c11 -O2 -Wall -Wextra -Werror \
    ...
    "$project_root/source/world_scene.c" \
    "$project_root/tests/test_world_scene.c" \
    $zlib_objects -lm -o "$test_binary"
