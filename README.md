...
M0 is the Homebrew shell. M1 pins the toolchain. M2 pins PaperBoat 1.0.1 and
forbids compiling it until M5:

- ARM11 / libctru lifecycle
- pinned Docker digest + makerom SHA
- `.3dsx` and `.3ds` CI artifacts with `build-info.txt`
- `upstream/PAPERBOAT.lock` + portability classification in `docs/M2.md`

**M0 acceptance still needs a Folium/hardware boot.** M1 is the build contract.
M2 is the dependency contract.
...
audit is [docs/M2.md](docs/M2.md). No later milestone will be marked complete
without evidence.
